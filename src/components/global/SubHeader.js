    ...
    <ul className="flex justify-start mt-5">
      {ABOUT_US_NAV_ITEMS.map((item) => (
        <li
          ...
          className={`flex items-center cursor-pointer md:ml-8 text-sm font-semibold py-2 transition-all duration-300 ease-in-out transform hover:scale-95 hover:text-[#3b82f6] 
            ${
        ...
        >
          ...
          <p className="no-underline">{item.navItem}</p>
        </li>
      ))}
    </ul>
  );
