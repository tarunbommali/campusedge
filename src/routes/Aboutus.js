...
import { ABOUT_JNTUGV, VISION_JNTUGV, MISSION_JNTUGV, ABOUT_US_NAV_ITEMS, DEPARTMENT_DETAILS, WEBSITE_DETAILS } from '../utils/constants'
        ...
        {ABOUT_US_NAV_ITEMS.map((item) => (
          <li 
            ...
            className={`cursor-pointer text-lg font-semibold px-4 py-2 
              ${activeNavId === item.navId ? 'underline  decoration-2 decoration-[#3b82f6] underline-offset-4' : 'text-gray-700 hover:text-[#4a4a4b]'}`}
          >
            {item.navItem}
          </li>
        ))}
